...
from models import BlogArticleForm, Article, EmailForm, PasswordForm, ContactUsForm
import db
...
def forgotpassword():
	...
	if request.method=='POST':
		...
		if user:
			...
			msg = Message('Password Change Request', recipients = [user['Email']])
			msg.html = render_template('/reset-email.html', username=user['Firstname'], link=url)
			mail.send(msg)
			return render_template('home.html')
		else:
...
def contact():
	...
	if request.method=='POST':
		...
		msg = Message(form.subject.data, recipients = ['canyouhearmeservice@gmail.com'])
		msg.html = render_template('/contact-email.html', name=form.name.data, email=form.email.data, message=form.message.data)
		mail.send(msg)
		return render_template('home.html')
